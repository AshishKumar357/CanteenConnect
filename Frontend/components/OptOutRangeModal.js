...
export default function OptOutRangeModal({ visible, onClose, onConfirm, defaultDate, dates = [] }) {
  ...
  function confirm() {
    ...
    onConfirm && onConfirm({ multiple, startDate: internalDates[s], endDate: internalDates[e], reason });
    onClose && onClose();
  }
      ...
      <View style={styles.overlay}>
        <View style={[styles.card, { marginHorizontal: wp(10) }]}>
          ...
          <TouchableOpacity style={[styles.multRow, { paddingVertical: rs(6) }]} onPress={() => setMultiple(m => !m)}>
            <MaterialIcons name={multiple ? 'check-box' : 'check-box-outline-blank'} size={rs(18)} color={multiple ? theme.colors.accent : theme.colors.muted} />
            <Text style={[styles.multText, { fontSize: Math.max(13, rs(13)) }]}>Allow multiple days</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.picker, { padding: rs(10), marginTop: 6 }]} onPress={() => setReasonOpen(r => !r)}>
            <Text style={{ fontSize: Math.max(13, rs(13)) }}>{reason || 'Select a reason'}</Text>
            <MaterialIcons name={reasonOpen ? 'keyboard-arrow-up' : 'keyboard-arrow-down'} size={rs(18)} />
          </TouchableOpacity>

          {reasonOpen && (
            <View style={styles.options}>
              {REASONS.map(r => (
                <TouchableOpacity key={r} onPress={() => { setReason(r); setReasonOpen(false); }} style={styles.option}>
                  <Text>{r}</Text>
                </TouchableOpacity>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  dateCard: { width: 64, height: 64, borderRadius: 10, backgroundColor: theme.colors.card, marginRight: 8, alignItems: 'center', justifyContent: 'center' },
  dateCardSel: { backgroundColor: theme.colors.accent, shadowColor: theme.colors.accent, shadowOpacity: 0.3, shadowOffset: { width: 0, height: 8 }, shadowRadius: 12, elevation: 6 },
  dateLabelSmall: { fontSize: 12, color: theme.colors.text },
  dateDaySmall: { fontSize: 18, fontWeight: '700' },
  picker: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 8, backgroundColor: theme.colors.background },
